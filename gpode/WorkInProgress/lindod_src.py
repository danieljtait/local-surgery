...
class MGPLinearAdapGrad3:
    ...
    def _phi_k_mh_update(self, k):
        ...
        xk = self._X[:, k]
        fk = self._dXdt(self._X, self._As, self._Gs)[:, k]
        ...
        def _lgp_hyperpar_pdf(val):
            ...
            Lxx, mdx_x, Cdxdx_x = _x_gp_pars(new_val,
                                             ...
                                             self.data.time,
                                             self._X[:, k])
            return _log_eq20_k(xk, fk, mdx_x,
                               Lxx, Cdxdx_x,
                               self._gammas[k],
                               phi_k_val=val, phi_k_prior=kpar.prior)

        try:
            lpnew = _lgp_hyperpar_pdf(new_val)
            lpcur = _lgp_hyperpar_pdf(kpar.value())
            A = np.exp(lpnew-lpcur)
            if np.random.uniform() <= A:
                for p, x in zip(kpar.parameters.values(), new_val):
                    p.value = x
        except:
            pass
...
#####
# helper function for utility, returns the quadratic form
#
#    - 0.5 x^T C^{-1} x
#
